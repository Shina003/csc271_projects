...
function updateTotals() {
    ...
    const totalIncome = Array.from(items).reduce((sum, item) => {
    ...
    document.getElementById('total-cost').textContent = `$${totalIncome}`;
}
...
function addItem(title, amount, note, date = new Date().toISOString().split('T')[0], addToLocalStorage = true) {
    ...
    if (addToLocalStorage) {
        ...
        incomeData.push({ date, title, amount, category: note });
...
function displayInitialItems() {
    const savedItems = JSON.parse(localStorage.getItem('incomeData')) || [];
    savedItems.forEach(item => addItem(item.title, item.amount, item.category, item.date, false)); // Do not add to localStorage

     // If localStorage is empty, add initialItems
    if (savedItems.length === 0) {
        initialItems.forEach(item => addItem(item.title, item.amount, item.note));
    } else {
        savedItems.forEach(item => addItem(item.title, item.amount, item.category, item.date, false));
    }
}
